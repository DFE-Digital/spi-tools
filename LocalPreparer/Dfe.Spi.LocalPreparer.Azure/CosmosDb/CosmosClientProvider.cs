...
namespace Dfe.Spi.LocalPreparer.Azure.CosmosDb
{
    public abstract class CosmosClientProvider : ICosmosClientProvider, IDisposable
    {
        ...
        public CosmosClientProvider(
            IOptions<SpiSettings> configuration, IContextManager contextManager, CosmosConnectionType connectionType)
        {
        ...
        private async Task<string?> GetAzureCosmosKeyAsync()
        {
            ...
            {
                Interactions.RaiseError(new List<string>() { "Failed to retrieve CosmosDb Account Access Key!" }, null);
                return null;
            }
        }
    }
