    ...
    public class AzureStorageService : IAzureStorageService
    {
        ...
        public async Task CopyTableToBlobAsync(ServiceName serviceName)
        {
            try
            {
                ...
                if (!CheckConnections(serviceName))
                    return;

                // Create a temporary blob container for table blobs
                CreateBlobContainerAsync(serviceName, tempContainerName);
                ...
                foreach (var tableName in _configuration.Value.Services.GetValueOrDefault(serviceName).Tables ?? new string[] { })
                {
                    ...
                    var succeeded = AzCopyLauncher.Run(arguments, _logger);
                    if (!succeeded)
                        break;
        ...
        public async Task CreateQueuesAsync(ServiceName serviceName)
        {
            try
            {
                ...
                foreach (var queueName in _configuration.Value.Services.GetValueOrDefault(serviceName).Queues ?? new string[]{ })
                {
                    ...
                    CreateQueueAsync(serviceName, queueName);
        ...
        private void CreateBlobContainerAsync(ServiceName serviceName, string blobContainerName)
        {
            try
            {
        ...
        private void CreateQueueAsync(ServiceName serviceName, string queueName)
        {
            try
            {
        ...
        public async Task CopyBlobAsync(ServiceName serviceName)
        {
            try
            {
                ...
                {
                    CreateBlobContainerAsync(serviceName, blobContainerName);
        ...
        public bool CheckConnections(ServiceName serviceName)
        {
