...
using CommandLine;
using Dfe.Spi.HistoricalDataLoader.GiasLoaderConsoleApp;
...
namespace Dfe.Spi.HistoricalDataLoader.UkrlpLoaderConsoleApp
{
    class Program
    {
        static async Task RunAsync(CommandLineOptions options, ILogger logger, CancellationToken cancellationToken)
        {
            ...
            var providers = new ProviderLoader(options.DataDirectory, options.StorageConnectionString, logger);
            await providers.LoadAsync(cancellationToken);
        }
